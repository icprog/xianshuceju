...
#define MPU6050_ID_2              0x69
#define MPU6050_DEVICE_ADDRESS_2  0xD1
...
MPU6050_RAW_DATA    MPU6050_Raw_Data2; 
MPU6050_REAL_DATA   MPU6050_Real_Data2;
...
//ID为陀螺仪id
int MPU6050_ReadData(uint8_t ID)
{
    u8 buf[14],buf2[14];
//    float Gyro_x_temp, Gyro_y_temp,Gyro_z_temp;
//	  float Gyro_x_temp2, Gyro_y_temp2,Gyro_z_temp2;
	if(ID==1)
	{
    ...
    else
    {
        ...
        MPU6050_Real_Data.Gyro_X = (float)(MPU6050_Raw_Data.Gyro_X )/65.5;     //见datasheet 32 of 47
        MPU6050_Real_Data.Gyro_Y = (float)(MPU6050_Raw_Data.Gyro_Y )/65.5;     //见datasheet 32 of 47
        MPU6050_Real_Data.Gyro_Z = (float)(MPU6050_Raw_Data.Gyro_Z )/65.5;     //见datasheet 32 of 47

    } 
	}
	else
	{
		    if(IIC_ReadData(MPU6050_DEVICE_ADDRESS_2,MPU6050_DATA_START,buf2,14) == 0xff)
    {
        printf("error 1J\r\n");
        return 0xff;
    }
    ...
    {
        //读取寄存器原生数据
        MPU6050_Raw_Data2.Accel_X = (buf2[0]<<8 | buf2[1]);
        MPU6050_Raw_Data2.Accel_Y = (buf2[2]<<8 | buf2[3]);
        MPU6050_Raw_Data2.Accel_Z = -(buf2[4]<<8 | buf2[5]); 
        MPU6050_Raw_Data2.Temp =    (buf2[6]<<8 | buf2[7]);  
        MPU6050_Raw_Data2.Gyro_X = (buf2[8]<<8 | buf2[9]);
        MPU6050_Raw_Data2.Gyro_Y = (buf2[10]<<8 | buf2[11]);
        MPU6050_Raw_Data2.Gyro_Z = -(buf2[12]<<8 | buf2[13]);

        //将原生数据转换为实际值，计算公式跟寄存器的配置有关
        MPU6050_Real_Data2.Accel_X = (float)(MPU6050_Raw_Data2.Accel_X)/8192.0; //见datasheet 30 of 47
        MPU6050_Real_Data2.Accel_Y = (float)(MPU6050_Raw_Data2.Accel_Y)/8192.0; //见datasheet 30 of 47
        MPU6050_Real_Data2.Accel_Z = (float)(MPU6050_Raw_Data2.Accel_Z)/8192.0; //见datasheet 30 of 47
        MPU6050_Real_Data2.Temp =   (float)(MPU6050_Raw_Data2.Temp)/340.0+36.53;//见datasheet 31 of 47
        MPU6050_Real_Data2.Gyro_X = (float)(MPU6050_Raw_Data2.Gyro_X)/65.5;     //见datasheet 32 of 47
        MPU6050_Real_Data2.Gyro_Y = (float)(MPU6050_Raw_Data2.Gyro_Y )/65.5;     //见datasheet 32 of 47
        MPU6050_Real_Data2.Gyro_Z = (float)(MPU6050_Raw_Data2.Gyro_Z)/65.5;     //见datasheet 32 of 47
